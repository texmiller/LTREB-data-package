...
data {
  ...
  // int<lower=0,upper=1> origin_01[N];          // plant origin status for growth model (not used for seedlings as all recruits)
}
...
transformed parameters {
  ...
  for(n in 1:N){
    predG[n] = exp(beta0[spp[n]] + betaendo[spp[n]]*endo_01[n]
