...
We modeled the mean and variance effects on each vital rate by fitting generalized linear mixed models (GLMM) to the long-term data with year and plot random effects. We fit all vital rate models in a Bayesian framework using Rstan, allowing us to propagate uncertainty from the vital rate estimates to our population model \citep{elderd2016quantifying}. 

For all species, the population model includes a 1 year reproductive delay. We account for this by modeling seedling growth and survival separately from adult growth and survival. Seedlings are those plants that are recruited into the plot in a given year, and typically have only one tiller. 
...
For example, growth of a given individual in year t+1 is modeled as:
\begin{equation} 
\label{eq:growth}
\begin{aligned}
...
+ \tau + \rho \\
\tau \sim N(0,\sigma_{s,e,t}) \\
\rho \sim N(0,\sigma_{p})
\end{aligned}
\end{equation}

Similarly, survival in year t+1 is modeled as:
\begin{equation} 
\label{eq:survival}
\begin{aligned}
