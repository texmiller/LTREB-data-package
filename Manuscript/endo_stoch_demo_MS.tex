...
Armed with this demographic data, we next constructed size-structured, stochastic population models. This demographic model describes transitions between sizes (measured as a count of tillers) from one year to the next.
...
where $\mathbf{n}_{t+1}$ is a vector distributed across plant size in year t+1 for each species and endophyte status.
